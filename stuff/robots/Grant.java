	...
	*/
	public void run() {
		...
		while(true) {
			...
			if (getEnergy() < 30) {
				try {
					sendMessage("aje.Phil", new String(getX() + " " + getY() + " I'm croakin' 'ere brov."));
	...
	public void onHitByBullet(HitByBulletEvent hbbe) {
		...
		if (getEnergy() < 70) {
			try {
				sendMessage("aje.Phil", new String(getX() + " " + getY() + " I'm gettin' pasted 'ere brov."));
			} catch (java.io.IOException ioe) {
	...
	public void onCustomEvent(CustomEvent ce) {
		stop();
		wallRighting = true;
       	Condition c = ce.getCondition();
        if (c.getName().equals("UpperLeftLimit")) {
			stop();	
			setTurnLeft(5);
			back(100);
			execute();
